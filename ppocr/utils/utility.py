...
def get_image_file_list(img_file):
    ...
    if os.path.isfile(img_file):
        ...
        for single_file in os.listdir(img_file):
            ...
            imgs_lists.append(single_file)
        temp = [(file.split('.')[0], file.split('.')[1]) for file in imgs_lists]
        temp = sorted(temp, key=lambda x: eval(x[0]))
        imgs_lists = [os.path.join(img_file, name[0] + '.' + name[1] ) for name in temp]
    if len(imgs_lists) == 0:
        raise Exception("not found any img file in {}".format(img_file))
    return imgs_lists
